...
public class LoggingAspect {
     ...
     */
    @Pointcut("execution(public int com.cmy.spring.aop.ArithmeticCalculator.*(..))")
    ...
    *在后置通知中不能访问目标方法执行的结果。
     * @param joinpoint
     */
//    @After("execution(public int com.cmy.spring.aop.ArithmeticCalculator.*(int, int))")
    ...
    *可以访问到方法的返回值。
     ...
     */
    @AfterReturning(value="execution(public int com.cmy.spring.aop.ArithmeticCalculator.*(..))", returning="result")
    ...
    *假如把参数写成NullPointerException则只在出现空指针异常的时候执行。
     ...
     * @param e
     */
    @AfterThrowing(value="execution(public int com.cmy.spring.aop.ArithmeticCalculator.*(..))", throwing="e")
